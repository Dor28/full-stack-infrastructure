/*****************
* File name: administration_add.sql
* Author: Eldor
* Date: 2021-06-27
* Version: 1.0
*************
...
CREATE OR REPLACE FUNCTION administration_add(in_first_name TEXT DEFAULT NULL, in_last_name TEXT DEFAULT NULL,
...
$$
DECLARE
    FUNC_NAME TEXT := 'administration_add';
    ERR_CODE  INT  := 4001;
...
BEGIN
    ...
    IF EXISTS(SELECT 1 FROM administration WHERE user_id = in_user_id) THEN
        RAISE EXCEPTION 'Error --> % % params %, %, %, %', ERR_CODE, FUNC_NAME, in_first_name, in_last_name, in_father_name, in_user_id
            USING HINT = 'a administration with the user_id already exists';
    END IF;

    RETURN QUERY INSERT INTO administration (first_name, last_name, father_name, user_id) VALUES (in_first_name, in_last_name,
                                                                                            in_father_name, in_user_id)
        RETURNING id_administration, first_name, last_name, father_name, user_id, reg_date, mod_date;
